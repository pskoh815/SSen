...
전략 로직:
  1. 후보 종목 풀 (2026-06-22 A/B 테스트로 교체 — 기존 압도적 주도테마/강세 주도종목·
     이벤트(60일 룩백) 대비 avg_pnl/total_pnl/MDD 전부 개선 확인):
     최근 POOL_LOOKBACK_BARS(120)거래일 이내 거래대금이 POOL_AMOUNT_THRESHOLD(1,000억원)
     이상인 날이 1회 이상 있는 종목 (날짜별로 평가 — 신호일(N) 기준 최근 120거래일 내
     1회라도 해당되면 후보)
  2. 아래 5개 조건이 모두 충족되는 날(N)을 매수 신호일로 채택, 당일 종가에 매수:
       Condition1: 최근 NARROW_RANGE_BARS(5)봉 모두 (고가-저가)/저가*100 <= NARROW_RANGE_PCT(5%)
                   ...
                   AND 종가 >= 시가*1.01 (2026-06-23 A/B 테스트로 추가 — win_rate +0.43%p,
                   total_pnl +107.68%→+147.19% 개선, avg_pnl/MDD는 소폭 trade-off 확인)
       Condition2: mid9 > mid26 AND mid26 > mid52 AND EMA(C,5) > mid9
                   (mid_N = (Highest(H,N)+Lowest(L,N))/2, 2026-06-22~23 A/B 테스트로 단계적
                   교체/추가 — 기존 Lowest(L,15)>(Highest(H,60)+Lowest(L,60))/2 AND
                   Lowest(L,20)>EMA(C,60) → EMA(C,5)>mid26>mid52 → mid9>mid26>mid52 →
                   mid9>mid26>mid52 AND EMA(C,5)>mid9 순으로 win_rate/avg_pnl/total_pnl/MDD
                   전부 단계적으로 개선 확인)
...
# Condition2 (2026-06-22 A/B 테스트로 단계적 교체):
#   1차: Lowest(L,15)>mid60 AND Lowest(L,20)>EMA60 → EMA(C,5)>mid26>mid52
#        (avg_pnl -0.19%→+3.96%, total_pnl +2.4%→+36.04%, MDD -21.33%→-18.88% 개선)
#   2차: EMA(C,5)>mid26>mid52 → MID(9)>mid26>mid52
#        (avg_pnl +3.96%→+4.38%, total_pnl +36.04%→+69.49%, MDD -18.88%→-12.59% 추가 개선)
#   mid_N = (Highest(H,N)+Lowest(L,N))/2
MID9_WINDOW  = 9     # mid9  = (Highest(H,9)+Lowest(L,9))/2
MID26_WINDOW = 26    # mid26 = (Highest(H,26)+Lowest(L,26))/2
MID52_WINDOW = 52    # mid52 = (Highest(H,52)+Lowest(L,52))/2
LOW20_WINDOW    = 20    # USAF(매도신호)의 국면판별 range_pct20 산출용 — Condition2와 무관
...
# ── 후보 종목 풀 파라미터 (2026-06-22 A/B 테스트로 교체 — 기존 압도적 주도테마/강세
#    주도종목·이벤트(60일 룩백) 대비 avg_pnl +4.38%→+6.21%, total_pnl +69.49%→+107.68%,
#    MDD -12.59%→-9.78% 전부 개선 확인) ───────────────────────────────────────
POOL_AMOUNT_THRESHOLD = 100_000_000_000  # 거래대금 1,000억원
POOL_LOOKBACK_BARS    = 120  # 후보 풀 판정 룩백 기간(거래일): 최근 N거래일 내 1회 이상 해당 시 후보로 인정
...
def _compute_signal_indicators_wide(
    ...
    narrow = range_pct.rolling(NARROW_RANGE_BARS).max() <= NARROW_RANGE_PCT
    # Condition1 추가 조건 (2026-06-23 A/B 테스트로 추가 — win_rate +0.43%p, total_pnl
    # +107.68%→+147.19% 개선 확인, avg_pnl/MDD는 소폭 trade-off): 종가가 시가보다 1%
    # 이상 상승한 날만 신호 인정 (변동성 수축 + 강한 양봉 마감)
    strong_close = close >= open_ * 1.01
    cond1 = narrow & strong_close

    ema5  = close.ewm(span=5, adjust=False).mean()
    mid9  = (high.rolling(MID9_WINDOW).max() + low.rolling(MID9_WINDOW).min()) / 2
    mid26 = (high.rolling(MID26_WINDOW).max() + low.rolling(MID26_WINDOW).min()) / 2
    mid52 = (high.rolling(MID52_WINDOW).max() + low.rolling(MID52_WINDOW).min()) / 2
    low20  = low.rolling(LOW20_WINDOW).min()  # USAF range_pct20 전용 (Condition2와 무관)
    ...
    # Condition2 추가 조건 (2026-06-23 A/B 테스트로 추가 — win_rate +3.53%p, avg_pnl
    # +5.64%→+6.81%, total_pnl +147.19%→+149.58%, MDD -15.43%→-13.01% 전부 개선 확인)
    cond2 = (mid9 > mid26) & (mid26 > mid52) & (ema5 > mid9)
...
def api_supertrend_trades(start: date, end: date) -> dict[str, Any]:
    ...
    amount_mat = ohlcv.pivot(index="date", columns="code", values="amount").sort_index()
    open_mat = ohlcv.pivot(index="date", columns="code", values="open").sort_index()
    ...
    # ── 후보 종목 풀: 최근 POOL_LOOKBACK_BARS거래일 이내 거래대금 POOL_AMOUNT_THRESHOLD
    #    이상이 1회 이상 발생한 종목만 신호 탐색 대상으로 인정 (날짜별 판정)
    pool_ok_mat = (amount_mat > POOL_AMOUNT_THRESHOLD).rolling(POOL_LOOKBACK_BARS, min_periods=1).max().astype(bool)
    qualifying_codes = set(pool_ok_mat.columns[pool_ok_mat.any(axis=0)])
    ...
    for code, g in ohlcv.groupby("code"):
        ...
        if code in pool_ok_mat.columns:
            pool_ok = pool_ok_mat[code].reindex(trading_dates, fill_value=False).to_numpy()
        else:
